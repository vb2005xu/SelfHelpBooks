<?php
		...
		if (!$session_id or !$zlid) {require('../muban/tz_login.php');	exit;}
		else {
		if ($check1==null){
		...
		else {$result3 = mysql_query("UPDATE  `evabioyetian`.`wenzhang` SET  `addtime` =  '$time' WHERE  `wenzhang`.`userid`=$session_id and `wenzhang`.`zl_wzid` =$zlid;");
