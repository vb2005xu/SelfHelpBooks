...
","", $row[3])."'; ";		
					  ...
					  if ($row[10]==0){
						  if ( $row[0]==$row[1]){
							  echo "yc1[".$i."]='<span style=".$syh."color:black".$syh.">来自</span><a href=book.php?id=".$row[8]." >《". $row[0]."》</a>'; "; 
						  } 
						  else {
							  echo "yc1[".$i."]='<a href=book.php?id=".$row[8]." >《". $row[0]."》</a><span style=".$syh."color:black".$syh.">转自</span><a href=book.php?id=".$row[11]." >《". $row[1]."》</a>'; ";
						  }	
					  }else {
						   echo "yc1[".$i."]='". $row[0]."(该用户已注销)'; ";
					  }		
					  echo "wzid1[".$i."]='".$row[7]."'; " ;
...
?>
		  function paiban_i() {
							   ...
							   n=Math.ceil(s/654);
							   if (str1.length==0) { 
									  $(".fengdi").before('<div style="text-align:center"><h3>还没有订阅呢，快到别人的书的封面，点订阅吧</h3></div>');
							   }	
							   else {					
									 for (i1=0; i1<str1.length; i1++){	
									  if (i1==(str1.length-1) ){ 
												  bu2='<br><br><div style="width:100%;text-align:center"><span class="yuanc" ><a href="i.php?=<?php echo '&bu='.($bu1-1); ?>" >>>>>>更多>>>>></a></span><div>';
									  } else{ 
												  bu2='';
									  }	
									  userid=userid1[i1];
									  ...
									  for (i=1;i<ye;i++){
											  if (i!==1){ 
											   ...
											   yc=yc1[i1];
											   if (i==ye-1){ 
													  zl='<br><span class="yuanc">'+yc+'</span>'+'<div class="eva-zl">'+zlc+'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+plc+'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'+gm+'</div>';
											   } else{ 
													  zl='';
											   }
											  if (str.match('_img_')) {
												  var str2=str.replace(/_img_/, "");c1='<a target="_blank" href="img.php?v=pic/'+str2+'"><img src="ui/ui_pic/transparent.gif" style="background-image:url(pic/'+str2+')"/></a>';c=addtime+'<p>'+c1+zl+'</p>';} 
											  else {
												  c=addtime+'<p>'+str.slice(n*(i-1),n*i)+zl+'</p>';
											  }
										   $(".fengdi").before('<div class="eva-wz">'+c+bu2+'</div>');
									   }		
									 }				
								}
						         $("#mybook").booklet( {
								...
								$(".b-wrap-left").prepend('<h3 class="eva-top"><a href="i.php">自助书，网页app&nbsp;&nbsp;&nbsp;&nbsp;今日订阅</a></h3>');	
		  ...
		  $(window).resize(function(){
				  ...
				  $('#mybook').replaceWith('<?php require('muban/i_bottom.php');	?>');
		          bbbbbb=paiban_i();
		  });//resize		 
