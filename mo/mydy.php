...
function paiban_p() {
if (fm_name1.length==0*1){$(".fengdi").before('<div></div><div style="text-align:center"><h3>还木有订阅某人呢。。。。。</h3></div>');}else {
for (i1=0; i1<fm_name1.length; i1++)
{
if (i1==(fm_name1.length-1) ){ bu2='<br /><div style="width:100%;text-align:center"><span class="yuanc" ><a href="mydy.php?=<?php echo '&bu='.($bu1-1); ?>" >>>>>>更多>>>>></a></span><div>'} else{ bu2=''};			
<?php require('muban/chinese_bu.php');	?>	
		...
		c='<div class="eva-book-s"><div style="width:90%;margin:5%;"><a href="book.php?id='+userid+'"><img src="../ui/ui_pic/transparent.gif" style="background-image:url(../fm_pic/'+fm_pic+');" /></a></div><div style="color:#000"><h3><a style="cursor:default">&nbsp;&nbsp;&nbsp;</a></h3><h3><a href="book.php?id='+userid+'">'+fm_name+'('+bu+')</a></h3>作者:'+name+'<br>订阅数:'+dy_count+'</div></div>';
		qx='<div class="eva-zl"><br><span class="qxdy"><a action-data="'+userid+'" href="###" >取消订阅</a></span></div>';
		 $(".fengdi").before('<div></div><div class="eva-wz">'+c+qx+bu2+'</div>');
