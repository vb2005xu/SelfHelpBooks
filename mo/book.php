<?php 
		...
		if ($user_row[0]==null) {
   			die('<div style="width:45%;height:45%;position:absolute">您访问的用户不存在<script language="javascript"> window.location.href="index.php";</script></div>');
		}
...
function paiban_i() {
		...
		n=Math.ceil(s/340);	
		if (str1.length==0*1){ 
				$(".fengdi").before('<div></div><div class="eva-wz"><p>作者还没有写文章呢.....</p></div>');
		}else{
			   for (i1=0; i1<str1.length; i1++){	
					  ...
					  str=str1[i1]; 
					  if (!dl) {
						  zlc='<span class="yuanc"><a class="log" >转藏('+zlc1[i1]+')</a></span>';
					  } else {
		  ...
		  <?php require_once('muban/2code.php');?>
