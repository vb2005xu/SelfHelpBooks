...
while ($pl_row=mysql_fetch_row($pl)){
		if (strpos($pl_row[0],'_img_')!==false){
			$c=str_replace("_img_","",$pl_row[0]);$aa='<div style="width:100%;text-align:center;background-color:#ddd" class="i_pl"><img src="ui/ui_pic/transparent.gif" style="background-image:url(pic/'.$c.')"/><span class='.$syh.'yuanc'.$syh.'><a href='.$syh.'pl.php?wzid='.$pl_row[4].$syh.'>(详细)</a></span></div>';
		} 
		else {
			$aa=substr($pl_row[0],0,30).'.....<span class='.$syh.'yuanc'.$syh.'><a href='.$syh.'pl.php?wzid='.$pl_row[4].$syh.'>(详细)</a></span>';
		}
...
?>	
	function paiban_p() {
			...
			if (!pln[0]) { 
					$(".fengdi").before('<div style="text-align:center">还没有评论呢，赶紧体验一下去吧</div>');
			}
			for (i=0;i<ye;i++){	
					 if (i==ye-1 ){ 
					bu='<div style="width:100%;text-align:center"><span class="yuanc" ><a href="i_pl.php?bu=<?php echo ($bu1-1); ?>" >>>>>>更多>>>>></a></span><div>';
			  } else{ 
					bu='';
			  }
