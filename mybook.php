<?php 
		...
		if ($user_row[0]==null) {
			$dyh="'";
			die('<div style="top:45%;left:45%;position:absolute">请登录<script language="javascript">setTimeout('.$dyh.'window.location.href="index.php"'.$dyh.',2000);</script></div>');
		}
...
","",$row[1])."'; " ;
		...
		function paiban_i() {
				...
				n=Math.ceil(s/654);		
				if (str1.length==0*1){ 
						$(".fengdi").before('<div class="eva-wz"><p>我还木有写文章呢，去写，或者转藏吧！</p></div>');
				}else{		
					   for (i1=0; i1<str1.length; i1++){		
							...
							for (i=1;i<ye;i++){
								 if (str.match('_img_')) {
									 var str2=str.replace(/_img_/, "");c='<a target="_blank" href="img.php?v=pic/'+str2+'"><img src="ui/ui_pic/transparent.gif" style="background-image:url(pic/'+str2+')"/></a>';
									 var djtp='<span style="color:black">(点击图片看大图)</span><br>';
								 } else{
	  ...
	  $(window).resize(function(){
			  ...
			  $('#mybook').replaceWith('<?php require('muban/mybook_bottom.php');?>');
	          bbbbbb=paiban_i();
	  });//resize		 
